...
SquareMatrix<T, Container>::SquareMatrix(): m_size(0){
    m_data = new Container<T, Container>();
}
...
SquareMatrix<T,Container>::SquareMatrix(initializer_list<initializer_list<T>> matrix): SquareMatrix(){
    ...
    m_data = new Container<Comtainer<T>>();
...
template<typename T, template<typename> class Container>
T SquareMatrix<T, Container>::Get(size_t row, size_t col){
    return m_data->Get(row).Cet(col);
}

template<typename T, template<typename> class Container>
size_t SquareMatrix<T, Container>::GetSize(){
    return m_size;
}

template<typename T, template<typename> class Container>
void SquereMatrix<T, Container>::Set(size_t row, size_t col, T value){
    ...
    row_container.Set(row, value);
    m_data->Set(row,row_container)
}

template<typename T, template<typename> class Container>
SquareMatrix<T, Container> SquereMatrix<T, Container>::Sum(const SquereMatrix<T, Container>& other){
    if(m_size != other.size){
        throw MatrixSquereSizeException("Размеры должны совпадать");
    }

    SquereMatrix<T, Container> result(m_size);
...
template<typename T, template<typename> class Container>
SquareMatrix<T, Container> SquereMatrix<T, Container>::Multiply(T value) const{
...
SquareMatrix<T, Container> SquareMatrix<T, Container>::MultiplyCol(size_t col_number,T value) const;{
    ...
    for( size_t i = 0; i <= m_size; i++){
        Container<T> col = m_data.Get(i,col_number)*value;
        result.Set(i, col_number,col );
    }
    return result;
}

template<typename T, template<typename> class Container>
SquareMatrix<T, Container> SquareMatrix<T, Container>::SumRow(size_t row_n1, size_t row_n2) const{
    ...
    for(size_t i = 0; i <= m_size; i++){
        T sum_row = row1.Get(i)+row2.Get(i);
...
template<typename T, template<typename> Class Container>
SquareMatrix<T, Container> SquareMatrix<T, Container>::SumCol(size_t col_n1, size_t col_n2) const{
    ...
    SquareMatrix<T, Container> result(*this);
    for( size_t i = 0; i < m_size; i++){
        T sum_col = Get(i, col_n1) + Get(i,col_n2);
